...
module Shamu
  module Entities
    ...
    class Entity
      ...
      def id
        raise("No id attribute defined. Add `attribute :id, on: :record` to #{self.class.name}")
      end
      ...
      def redact!(*attributes)
        hash =
          if attributes.first.is_a?(Symbol)
            attributes.zip([nil] * attributes.length).to_h
          else
            attributes.first
          end
      ...
      private
        ...
        class << self
          ...
          private
            ...
            def attr_accessor(*)
              raise("Remember, an Entity is immutable. Use a Services::Service to mutate the underlying data.")
            end

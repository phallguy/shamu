  ...
  module Auditing
    ...
    class LoggingAuditingService < AuditingService
      ...
      def commit(transaction)
        logger.unknown("AUDIT TRANSACTION action: #{transaction.action} entity: #{transaction.entity_path} by user: #{transaction.principal.try(:principal_id_chain)} params: #{transaction.filtered_params}")
      end
    end
  end
end
